...
def detect_lanes_img(img):
    ...
    try:
        l_slope = (left_fit_line[3] - left_fit_line[1]) / (left_fit_line[2] - left_fit_line[0])
        r_slope = (right_fit_line[3] - right_fit_line[1]) / (right_fit_line[2] - right_fit_line[0])
        #y = l_slope * x - l_slope * left_fit_line[0] + left_fit_line[1]
        #y = r_slope * x - r_slope * right_fit_line[0] + right_fit_line[1]
        vanishing_point_x = (l_slope * left_fit_line[0] - left_fit_line[1] - (r_slope * right_fit_line[0] - right_fit_line[1])) / (l_slope-r_slope)
        vanishing_point_y = l_slope * vanishing_point_x - l_slope * left_fit_line[0] + left_fit_line[1]
        intersection_x = (left_fit_line[0] + right_fit_line[0])/2
        intersection_y = (left_fit_line[1] + right_fit_line[1])/2
        handling_slope = (vanishing_point_y - intersection_y) / (vanishing_point_x - intersection_x)
        #min_distance =  (pow(left_fit_line[0] - right_fit_line[0],2) + pow(left_fit_line[1]-right_fit_line[1],2)) **0.5
    except:
        handling_slope = 11
    # smoothing by using previous frames
    if np.abs(handling_slope) < 10 and (right_fit_line[2] - left_fit_line[2] > 100)  :
        print(handling_slope)
